...
export default function Products({data = []}: { data: IProductsProps[]}){
  if(!data.length) {
    ...
    <div className="flex flex-col items-center gap-6 md:flex-row md:flex-wrap md:justify-center">
      {data.length > 0 && (
        data.map((prod, index) => (
          <div key={index} className="flex font-suse flex-col items-center text-left w-3/4 md:w-fit hover:shadow-lg p-2 duration-600 transition-all">
            <Link href={`/product/${prod.name.toLowerCase().replace(/\s+/g, "_")}-${prod.id}`} className="flex items-center flex-col">
              ...
              <div className="w-full">
                <h1 className="font-semibold text-xl md:truncate md:text-base">{prod.name}</h1>
                <p>{prod.desc}</p>
